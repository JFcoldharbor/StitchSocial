...
data class CommunityPost(
    ...
    var body: String,
    /**
     * Reference to an existing Video doc in the user's library. Used by
     * `VIDEO_LINK` posts where the creator is sharing an already-published
     * video. Keep nil for direct uploads — those use [videoURL] instead.
     */
    var videoLinkID: String? = null,
    /**
     * Direct Storage download URL for video clip posts uploaded inline via
     * the community composer (path: `community-posts/{communityID}/{postID}.mp4`).
     * Distinct from [videoLinkID] which references another doc.
     */
