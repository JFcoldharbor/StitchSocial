 ...
 *
 * UPDATED: Now includes comprehensive hashtag query builders for video search and discovery
 */
...
object DocumentPaths {
    ...
    // MARK: - Query Builder Utilities (EXISTING)
    ...
    // MARK: - NEW HASHTAG QUERY BUILDERS

    /** Build query path for videos by hashtag */
    fun videosByHashtagQuery(hashtag: String): QueryPathBuilder {
        return QueryPathBuilder(videos)
            .whereField("hashtags", "array-contains", hashtag.removePrefix("#").lowercase())
            .orderBy("createdAt", descending = true)
    }

    /** Build query path for videos by hashtag with trending sort */
    fun videosByHashtagTrendingQuery(hashtag: String): QueryPathBuilder {
        return QueryPathBuilder(videos)
            .whereField("hashtags", "array-contains", hashtag.removePrefix("#").lowercase())
    ...
    /** Build query path for videos by hashtag with engagement sort */
    fun videosByHashtagEngagementQuery(hashtag: String): QueryPathBuilder {
        return QueryPathBuilder(videos)
            .whereField("hashtags", "array-contains", hashtag.removePrefix("#").lowercase())
            .orderBy("engagementRatio", descending = true)
            .orderBy("createdAt", descending = true)
    }

    /** Build query path for hashtag discovery (parent threads only) */
    fun hashtagDiscoveryQuery(hashtag: String): QueryPathBuilder {
        return QueryPathBuilder(videos)
            .whereField("hashtags", "array-contains", hashtag.removePrefix("#").lowercase())
            .whereField("conversationDepth", "==", 0) // Parent threads only
            .orderBy("discoverabilityScore", descending = true)
            .orderBy("createdAt", descending = true)
    }

    /** Build query path for trending hashtag analysis (last 7 days) */
    fun trendingHashtagAnalysisQuery(): QueryPathBuilder {
        return QueryPathBuilder(videos)
            .whereField("createdAt", ">=", "TIMESTAMP_7_DAYS_AGO") // Will be replaced with actual timestamp
            .orderBy("createdAt", descending = true)
            .limit(500) // For hashtag frequency analysis
    }

    /** Build query path for hashtag + content type filtering */
    fun videosByHashtagAndTypeQuery(hashtag: String, contentType: String): QueryPathBuilder {
        return QueryPathBuilder(videos)
            .whereField("hashtags", "array-contains", hashtag.removePrefix("#").lowercase())
            .whereField("contentType", "==", contentType)
            .orderBy("createdAt", descending = true)
    }

    /** Build query path for user's videos with specific hashtag */
    fun userVideosByHashtagQuery(userId: String, hashtag: String): QueryPathBuilder {
        return QueryPathBuilder(videos)
            .whereField("creatorID", "==", userId)
            .whereField("hashtags", "array-contains", hashtag.removePrefix("#").lowercase())
            .orderBy("createdAt", descending = true)
    }

    /** Build query path for hashtag popularity ranking */
    fun hashtagPopularityQuery(hashtag: String): QueryPathBuilder {
        return QueryPathBuilder(videos)
            .whereField("hashtags", "array-contains", hashtag.removePrefix("#").lowercase())
    ...
    /** Build query path for recent hashtag usage */
    fun recentHashtagUsageQuery(hashtag: String, daysBack: Int = 7): QueryPathBuilder {
        return QueryPathBuilder(videos)
            .whereField("hashtags", "array-contains", hashtag.removePrefix("#").lowercase())
            .whereField("createdAt", ">=", "TIMESTAMP_${daysBack}_DAYS_AGO")
            .orderBy("createdAt", descending = true)
    }
    ...
    // MARK: - Cache Key Generation (UPDATED WITH HASHTAG SUPPORT)
    ...
    // NEW: Hashtag-specific cache keys

    /** Generate cache key for hashtag search */
    ...
    /** Generate cache key for hashtag popularity data */
    fun hashtagPopularityCacheKey(hashtag: String): String {
        return "stitchfin_hashtag_popularity_${hashtag.removePrefix("#").lowercase()}"
    }

    /** Generate cache key for hashtag analytics */
    fun hashtagAnalyticsCacheKey(hashtag: String, timeframe: String): String {
        return "stitchfin_hashtag_analytics_${hashtag.removePrefix("#").lowercase()}_$timeframe"
    }
    ...
    /** Validate hashtag query format */
    fun isValidHashtagPath(hashtag: String): Boolean {
        val cleanHashtag = hashtag.removePrefix("#").lowercase()
        return cleanHashtag.isNotEmpty() &&
                cleanHashtag.length <= 30 &&
                cleanHashtag.matches(Regex("^[a-zA-Z0-9_]+$"))
    }
    ...
    /** Get all hashtag query builder names */
    fun getAllHashtagQueryTypes(): List<String> {
        return listOf(
            "videosByHashtag", "videosByHashtagTrending", "videosByHashtagEngagement",
            "hashtagDiscovery", "trendingHashtagAnalysis", "videosByHashtagAndType",
            "userVideosByHashtag", "hashtagPopularity", "recentHashtagUsage"
        )
    }
...
// MARK: - Query Path Builder (ENHANCED)

/**
 * Helper class for building Firestore query paths with hashtag support
...
class QueryPathBuilder(private val basePath: String) {
    ...
    /** Add hashtag array-contains condition */
    fun whereHashtagContains(hashtag: String): QueryPathBuilder {
        val cleanHashtag = hashtag.removePrefix("#").lowercase()
        whereConditions.add("hashtags array-contains $cleanHashtag")
        return this
    }

    /** Add thread-only filter */
    fun threadsOnly(): QueryPathBuilder {
        whereConditions.add("conversationDepth == 0")
        return this
    }

    /** Add engagement threshold filter */
    fun minEngagement(threshold: Double): QueryPathBuilder {
        whereConditions.add("engagementRatio >= $threshold")
        return this
    }

    /** Add time range filter */
    fun newerThan(timestamp: String): QueryPathBuilder {
        whereConditions.add("createdAt >= $timestamp")
        return this
    }
    ...
    /** Check if query contains hashtag conditions */
    fun hasHashtagConditions(): Boolean {
        return whereConditions.any { it.contains("hashtags") }
    }

    /** Check if query is optimized for performance */
    fun isOptimized(): Boolean {
        val hasProperIndexes = whereConditions.size <= 3 && orderByConditions.size <= 2
        val hasReasonableLimit = limitValue == null || limitValue!! <= 50
        return hasProperIndexes && hasReasonableLimit
    }
}